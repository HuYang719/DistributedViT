...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[attention]") == 0) return MULTI_HEAD_ATTENTION;
    if (strcmp(type, "[layernorm]")==0) return LAYERNORM;
    if (strcmp(type, "[positional_embedding]") == 0) return POSITIONAL_EMBEDDING;
    if (strcmp(type, "[flatten]")==0) return FLATTEN;
    if (strcmp(type, "[linear]")==0) return MLP;
    if (strcmp(type, "[debug]")==0) return DEBUG_PRINT;
...
layer parse_shortcut(list *options, size_params params, network net)
{
    ...
    int c = option_find_int(options, "c", 0);
    int h = option_find_int(options, "h", 0);
    int w = option_find_int(options, "w", 0);
    ...
    layer s = make_shortcut_layer(params.batch, n, layers, sizes, w, h, c, layers_output, layers_delta,
    ...
    for (i = 0; i < n; ++i) {
        int index = layers[i];
        //  printf("Parse shortcut, p.w=%d, net.w=%d, p.h=%d, n.h=%d\n", w, net.layers[index].out_w, h, net.layers[index].out_h);

        // assert(params.w == net.layers[index].out_w && params.h == net.layers[index].out_h);
       
        if (w != net.layers[index].out_w || h != net.layers[index].out_h || c != net.layers[index].out_c)
...
layer parse_attention(list *options, size_params params)   // 03.29/2012 - lucyyang
{
    int head_num = option_find_int(options, "head_num", 0);
    int key_dim = option_find_int(options, "key_dim", 0);


    int batch,input_size,model_dim;
    input_size = params.inputs;

    batch=params.batch;

    // layer l = make_slice_layer(batch, w, h, c, slice_axis, slice_num, slice_pos);
    layer l = make_multi_head_attention_layer(batch, input_size, head_num, key_dim);
    return l;
}

layer parse_layernorm(list *options, size_params params)   // 03.29/2012 - lucyyang
{
...
layer parse_positional_embedding(list *options, size_params params)   // 03.29/2012 - lucyyang
{
    // int model_dim = option_find_int(options, "model_dim", 0);
    int model_dim = option_find_int(options, "model_dim", 1);
    int input_size =  option_find_int(options, "input_size", 0);
    ...
    // layer l = make_slice_layer(batch, w, h, c, slice_axis, slice_num, slice_pos);
    layer l = make_positional_embedding_layer(batch, input_size, model_dim);
    return l;
}
...
layer parse_flatten(list *options, size_params params)   // 03.29/2012 - lucyyang
{
    ...
    // model_dim = params.model_dim;
    layer l = make_flatten_layer(batch, model_dim, gh, gw);
    return l;
}
...
layer parse_debug(list *options, size_params params)   // 03.29/2012 - lucyyang
{
    // int model_dim = option_find_int(options, "model_dim", 0);
    int dim1 = option_find_int(options, "dim1", 0);
    int dim2 = option_find_int(options, "dim2", 0);
    int dim3 = option_find_int(options, "dim3", 0);
   
    int  batch = params.batch;
    // model_dim = params.model_dim;

    // layer l = make_slice_layer(batch, w, h, c, slice_axis, slice_num, slice_pos);
    layer l =   make_debug_print_layer(batch, dim1, dim2, dim3);
    return l;
}
...
{
    net->model_dim = option_find_int(options, "model_dim", 0);
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    while(n){
        ...
        if(lt == CONVOLUTIONAL){
            ...
            net.layers[l.index].use_bin_output = 0;
            net.layers[l.index].keep_delta_gpu = 1;
        }else if (lt == MULTI_HEAD_ATTENTION) {
            l = parse_attention(options, params );
        }else if (lt == LAYERNORM) {
            l = parse_layernorm(options, params);
        }else if (lt == POSITIONAL_EMBEDDING) {
            l = parse_positional_embedding(options, params);
        }else if (lt == DEBUG_PRINT) {
            l = parse_debug(options, params);
        }else if (lt == FLATTEN) {
            l = parse_flatten(options, params);
        }else if (lt == MLP) {
            l = parse_mlp(options, params);
        }else if (lt == SAM) {
